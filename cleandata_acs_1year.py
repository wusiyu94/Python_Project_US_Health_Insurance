...
############### county level data (for regression)
...
#################  state level data (for creating maps)

b0 = "DP03_0096PE,DP03_0097PE,DP03_0098PE,DP03_0099PE"
b1 = "DP03_0101PE,DP03_0106PE,DP03_0107PE,DP03_0108PE,DP03_0111PE,DP03_0112PE,DP03_0113PE,DP03_0116PE,DP03_0117PE,DP03_0118PE"

for year in range(2012,2017):
    e0_year = pd.DataFrame(data = get_data("state",year,b0)[1:],columns = get_data("state",year,b0)[0])
    e1_year = pd.DataFrame(data = get_data("state",year,b1)[1:],columns = get_data("state",year,b1)[0])
    
    data_year = pd.merge(e0_year,e1_year,how='left',on=['NAME','state'])
    
    filename = "acs_" + str(year) + "_state.csv"
    data_year.to_csv(filename)
